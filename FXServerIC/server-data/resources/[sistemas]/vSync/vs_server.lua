...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'O horário está ~b~congelado~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'O horário não está ~y~congelado~s~.')
            end
        else
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if blackout then
            print("Modo Blackout está ativado.")
        else
            print("Modo Blackout está desativado.")
        end
    else
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                TriggerClientEvent('vSync:notify', source, 'Modo Blackout foi ~b~ativado~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Modo Blackout foi ~r~desativado~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
