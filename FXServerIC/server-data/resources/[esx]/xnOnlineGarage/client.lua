                                    ...
                                    if (x > 0.5 or y > 0.5 or z > 0.5) or (x < -0.5 or y < -0.5 or z < -0.5) then
                                        Citizen.CreateThread(function()
                                            ...
                                            for i,veh in ipairs(xnGarage.vehicles) do
												print(json.encode(xnGarage.vehicles))
    ...
    while true do
        ...
        if not xnGarage.curGarage then
            for ln,location in pairs(xnGarageConfig.locations) do
				...
				print(hasProperty)
                if (Vdist2(GetEntityCoords(ent),ix,iy,iz) < 500.0) and hasProperty then
...
AddEventHandler('esx_2KGarage:reloadGarages', function()
	ESX.TriggerServerCallback('esx_2KGarage:getOwnedProperties', function(properties)
      userProperties = properties
            for zoneKey,zoneValues in pairs(xnGarageConfig.locations) do
                if zoneValues.Propriedade and has_value(userProperties, zoneValues.Propriedade) then
					print(json.encode(userProperties))
					print(json.encode(zoneValues.Propriedade))
                    hasProperty = true
					print(hasProperty)
              else
					 hasProperty = false
