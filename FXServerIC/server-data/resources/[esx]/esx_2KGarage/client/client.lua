...
local vehicle_esx       = GetVehiclePedIsIn(playerPed,  false) 
local xnGarage_default = {
	vehicleTaken = false,
...
function ToCoord(t,withHeading)
...
function CreateVehicleFromData(data, x,y,z,h, dontnetwork)
...
function LoadGarage(wait)
    Citizen.CreateThread(function()
        ...
        if vehicleTable and vehicleTable[xnGarage.curGarageName] then
			...
			--print(json.encode(vehicleTable[xnGarage.curGarageName]))
                                    ...
                                    if (x > 0.5 or y > 0.5 or z > 0.5) or (x < -0.5 or y < -0.5 or z < -0.5) then
                                        Citizen.CreateThread(function()
                                            ...
                                            SetEntityCoords(ent, x, y, z)
                                             ...
                                             -- Teleport vehicle to out side
                                            xnGarage.vehicleTaken = GetVehiclePedIsUsing(GetPlayerPed(-1))
                                            FreezeEntityPosition(xnGarage.vehicleTaken, true)
											SetEntityCoordsNoOffset(xnGarage.vehicleTaken,x+0.0001, y+0.0001, z+1.0, 0, 0, 1)
											SetEntityHeading(xnGarage.vehicleTaken, h)
											SetPedIntoVehicle(GetPlayerPed(-1), xnGarage.vehicleTaken, -1) -- Put the ped into the new vehicle
											SetVehicleOnGroundProperly(xnGarage.vehicleTaken) -- Set vehicle on ground
                                            Citizen.Wait(1000)
                ...
                if vehData ~= "none" then
                    ...
                    if GetEntityModel(xnGarage.vehicleTaken) ~= model then -- Don't display the vehicle we currently have out
                        ...
                        if WarMenu.Button(name) then
                            ...
                            if xnGarage.vehicleTaken then ESX.Game.DeleteVehicle(xnGarage.vehicleTaken) end
...
function GarageStart(Propriedades) 
	Citizen.CreateThread(function()
		while true do
	--	print(Propriedades)
			local isInVehicle, veh, vehModel = GetVehicle()
			if not xnGarage.curGarage then
				for ln,location in pairs(xnGarageConfig.locations) do
					...
					if Vdist2(GetEntityCoords(ent),ix,iy,iz) < location.inLocation[2]*2.5 and not IsPedSprinting(GetPlayerPed(-1)) then
						if not allowed then
							ESX.ShowHelpNotification("WEB_VEH_INV")
						else
							ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to enter your garage.")
							if IsControlJustReleased(0, 51) then
								...
								if not isInVehicle then
									...
									if xnGarage.vehicleTaken ~= veh then
										print(xnGarage.vehicleTakenLoc)
										print(xnGarage.vehicleTakenPos)
										--print(json.encode(ESX.Game.GetVehicleProperties(veh)))
									...
									if saveCallbackResponse == "no_slot" then
										...
										while Vdist2(GetEntityCoords(ent),ix,iy,iz) < location.inLocation[2]*2.5 do
											Citizen.Wait(0)
											ESX.ShowHelpNotification("WEB_VEH_FULL")
										end
									end
				...
				if Vdist2(GetEntityCoords(ent),ToCoord(gr.modifyMarker)) <= 1.5 then
					ESX.ShowHelpNotification("MP_MAN_VEH") -- native localisation is cool
